...
pub fn sync_with_models(base_url: &str, models: &[Value]) -> Result<()> {
    ...
    let new_content = rebuild(&config, base_url, models);
...
fn rebuild(config: &toml::Value, base_url: &str, models: &[Value]) -> String {
    ...
    let new_models: Vec<toml::Value> = models
        ...
        .map(|m| {
            ...
            t.insert("alias".into(), toml::Value::String(id));
            t.insert("temperature".into(), toml::Value::Float(0.2));
